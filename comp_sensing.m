...
mspe = [];
msie = [];
for f = [0.9,0.8,0.7,0.6,0.5,0.4,0.3,0.2,0.1,0.05]
    n = p*p;
    m = ceil(f*n);
    count = 0;
    ...
    %{
    boundary correction, only for stride = 1
    X_as = X;
    for i = 1:p-1
        for j = 1:p-1
            X(i,j)=p*p/(i*j)*X(i,j);
    ...
    imshow(uint8(X))
    X_all = [X_all; X(:)'];    
    mspe_i = I_patch - I_rec';
    mspe_i = mspe_i.^2;
    mspe = [mspe, mean(mean(mspe_i))];
    msie = [msie,mean(mean((double(I(8:248,8:248))-X(8:248,8:248)).^2))];
end
plot(msie,'b')
hold on
plot(mspe,'r-')
